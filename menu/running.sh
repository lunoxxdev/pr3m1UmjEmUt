...
dropbear__status=$(/etc/init.d/dropbear status | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
dropbear_status=$(systemctl status dropbear status | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
if [[ $sldns == "running" ]]; then
  status_sldns=" ${GREEN}Running ${NC}( No Error )"
else
  status_sldns="${RED}  Not Running ${NC}  ( Error )"
fi
...
# STATUS SERVICE  SQUID 
if [[ $udp == "running" ]]; then 
   status_udp=" ${GREEN}Running ${NC}( No Error )"
else
   status_udp="${RED}  Not Running ${NC}  ( Error )"
fi
...
# STATUS SERVICE DROPBEAR 
if [[ $drop == "running" ]]; then 
   drop=" ${GREEN}Running ${NC}( No Error )${NC}"
else
   drop="${RED}  Not Running ${NC}  ( Error )${NC}"
fi
...
echo -e "❇️ License     : Lifetime  "
echo -e "❇️ Version     : $(cat /opt/.ver) "
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m${NC}"
echo -e "\E[44;1;39m            ⇱ Service Information ⇲             \E[0m"
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m${NC}"
echo -e "❇️ SSH / TUN               :$status_openvpn"
echo -e "❇️ SSH SLOWDNS             :$status_sldns"
echo -e "❇️ UDP-COSTUM              :$status_udp"
...
echo -e "❇️ SSH WIREGUARD           :$status_pptp"
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m${NC}"
echo ""
